export default function getBrowser() {
    // Determine userAgent based on the environment
    let userAgent = "Unknown";
    // Client-side
    if (typeof navigator !== "undefined") {
        userAgent = navigator.userAgent;
    }
    // Server-side (requires global context or framework handling like Next.js)
    else if (typeof global !== "undefined" && global.headers) {
        userAgent = global.headers["user-agent"] || "Unknown";
    }
    const osArray = [
    ...
    const matchBrowser = browserArray.find((browser) => browser.regex.test(userAgent))?.name ||
        "Unknown Browser";
